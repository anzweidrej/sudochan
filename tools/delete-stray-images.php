...
foreach ($boards as $board) {
    ...
    foreach ($stray_src as $src) {
        ...
        if (!FileManager::file_unlink($board['dir'] . $config['dir']['img'] . $src)) {
            $er = error_get_last();
    ...
    foreach ($stray_thumb as $thumb) {
        ...
        if (!FileManager::file_unlink($board['dir'] . $config['dir']['thumb'] . $thumb)) {
            $er = error_get_last();
            die("error: " . $er['message'] . "\n");
        }
    }

    echo sprintf("deleted %s files (%s)\n", $stats['deleted'], Filesize::format_bytes($stats['size']));
}
