...
$(document).ready(function () {
    ...
    var hideLockedThread = function ($thread) {
        $thread.hide().addClass('hidden');
    };

    var restoreLockedThread = function ($thread) {
        $thread.show().removeClass('hidden');
    };
    ...
    $('hr:first').before(
        '<div id="toggle-locked-threads" style="text-align:right"><a class="unimportant" href="javascript:void(0)">-</a></div>',
    );
    $('div#toggle-locked-threads a')
        .text(
            hide_locked_threads
                ? _('Show locked threads')
                : _('Hide locked threads'),
        )
        .on('click', function () {
    ...
    $(document).on('new_post', function (e, post) {
        if (hide_locked_threads) {
            $(post)
                .find('img.icon[title="Locked"], i.fa-lock.fa')
                .each(function () {
                    hideLockedThread(getThreadFromIcon($(this)));
                });
        }
