...
(function () {
    ...
    var do_css = function () {
        ...
        var dummy_reply = $('<div class="post reply"></div>').appendTo(
            $('body'),
        );
    ...
    var show_quick_reply = function () {
        ...
        $postForm.find('table tr').each(function () {
            ...
            if ($th.length && $td.length) {
                ...
                if ($td.find('input[type="submit"]').length) {
                    ...
                    $('<td class="submit"></td>')
                        .append($td.find('input[type="submit"]'))
                        .insertAfter($td);
                }
                ...
                if ($td.find('#recaptcha_widget_div').length) {
                    ...
                    $('#recaptcha_response_field').on('focus', function () {
                        if (
                            ...
                            $postForm
                                .find('input[name="recaptcha_challenge_field"]')
                                .val($('#recaptcha_challenge_field').val());
                            $postForm
                    ...
                    var $newRow = $(
                        '<tr><td class="recaptcha-response" colspan="2"></td></tr>',
                    );
                    $newRow
                        .children()
                        .first()
                        .append($td.find('input').removeAttr('style'));
                    $newRow
                    ...
                    $('#recaptcha_response_field').addClass(
                        'recaptcha_response_field',
                    );

                    $td.replaceWith(
                        $('<td class="recaptcha" colspan="2"></td>').append(
                            $('<span></span>').append($captchaimg),
                        ),
                    );
                ...
                if ($td.find('input[type="file"]').length) {
                    if ($td.find('input[name="file_url"]').length) {
                        ...
                        $file_url
                            .clone()
                            .attr('placeholder', _('Upload URL'))
                            .appendTo($newRow.find('td'));
                    ...
                    $td.find('input[type="checkbox"]').each(function () {
                        if ($(this).attr('name') == 'spoiler') {
                            ...
                            $postForm
                                ...
                                .removeAttr('colspan')
                                .after(
                                    $('<td class="spoiler"></td>').append(
                                        this,
                                        ' ',
                                        $('<label for="q-spoiler-image">').text(
                                            _('Spoiler Image'),
                                        ),
                                    ),
                                );
        ...
        $postForm
            ...
            .removeAttr('id')
            .removeAttr('cols')
            .attr('placeholder', _('Comment'));

        $postForm
            .find('textarea:not([name="body"]),input[type="hidden"]')
            .removeAttr('id')
            .appendTo($dummyStuff);
        ...
        $origPostForm.find('textarea[name="body"]').on('focus', function () {
            ...
            if (settings.get('hide_at_top', true)) {
                $(window)
                    .on('scroll', function () {
                        ...
                        if (
                            $(this).scrollTop() <
                            $origPostForm.offset().top +
                                $origPostForm.height() -
                                100
                        )
                            $postForm.fadeOut(100);
                        else $postForm.fadeIn(100);
                    })
                    .trigger('scroll');
            } else {
                $postForm.show();
    ...
    var floating_link = function () {
        ...
        $(
            '<a href="javascript:void(0)" class="quick-reply-btn">' +
                _('Quick Reply') +
                '</a>',
        )
            .on('click', function () {
            ...
            if (settings.get('hide_at_top', true)) {
                ...
                $(window)
                    .on('scroll', function () {
                        ...
                        if (
                            $(this).scrollTop() <
                            $('form[name="post"]:first').offset().top +
                                $('form[name="post"]:first').height() -
                                100
                        )
                            $('.quick-reply-btn').fadeOut(100);
                        else $('.quick-reply-btn').fadeIn(100);
                    })
                    .trigger('scroll');
            }
        });
