...
$(document).ready(function () {
    ...
    var restoreImage = function () {
        $(this).attr('src', $(this).attr('data-orig')).removeClass('hidden');
    };
        ...
        if (hide_images) {
            $('a.hide-image-link').each(function () {
                ...
                $(this)
                    .hide()
                    .after(
                        '<span class="toggle-images-placeholder">' +
                            _('hidden') +
                            '</span>',
                    );
            });
    ...
    $('hr:first').before(
        '<div id="toggle-images" style="text-align:right"><a class="unimportant" href="javascript:void(0)">-</a></div>',
    );
    $('div#toggle-images a')
        ...
        .on('click', function () {
            ...
            if (hide_images) {
                $('img.post-image, .theme-catalog .thread>a>img').each(
                    hideImage,
                );
