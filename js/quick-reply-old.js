...
$(document).ready(function () {
    ...
    txt_new_reply =
        txt_new_topic == _('Submit') ? txt_new_topic : new_reply_string;
    ...
    $('div.post.op').each(function () {
        ...
        $('<a href="#">[' + _('Quick reply') + ']</a>')
            .insertAfter($(this).children('p.intro').children('a:last'))
            .on('click', function () {
                $('div.banner').remove();
                $(
                    '<div class="banner">' +
                        fmt(
                            _(
                                'Posting mode: Replying to <small>&gt;&gt;{0}</small>',
                            ),
                            [id],
                        ) +
                        ' <a class="unimportant" onclick="undo_quick_reply()" href="javascript:void(0)">[' +
                        _('Return') +
                        ']</a></div>',
                ...
                $(
                    '<input type="hidden" name="quick-reply" value="' +
                        id +
                        '">',
