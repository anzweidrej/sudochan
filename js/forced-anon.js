...
$(document).ready(function () {
    var force_anon = function () {
        if ($(this).children('a.capcode').length == 0) {
            ...
            old_info[id] = {
                name: p.children('span.name').text(),
                trip: p.children('span.trip').text(),
            };
    ...
    $('hr:first').before(
        '<div id="forced-anon" style="text-align:right"><a class="unimportant" href="javascript:void(0)">-</a></div>',
    );
    $('div#forced-anon a').text(
        _('Forced anonymity') +
            ' (' +
            (forced_anon ? _('enabled') : _('disabled')) +
            ')',
    );

    $('div#forced-anon a').on('click', function () {
        forced_anon = !forced_anon;

        if (forced_anon) {
            $('div#forced-anon a').text(
                _('Forced anonymity') + ' (' + _('enabled') + ')',
            );
            localStorage.forcedanon = true;
            enable_fa();
        } else {
            $('div#forced-anon a').text(
                _('Forced anonymity') + ' (' + _('disabled') + ')',
            );
    ...
    if (forced_anon) enable_fa();

    $(document).on('new_post', function (e, post) {
        if (forced_anon) $(post).find('p.intro label').each(force_anon);
    });
});
